        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.is_error == 0) {
                    $('#' + form_id).before('<div class="msg bg-success">' + response.msg + '</div><br/>');
                    $('#' + form_id)[0].reset();
                } else {
                    $('#' + form_id).before('<div class="msg bg-danger">' + response.msg + '</div><br/>');
                }
            }
        });
    }
...
function saveSubscriber(form_id) {
    if (formValidateOnly(form_id)) {
        var form_data = $('#' + form_id).serialize();
        form_data += '&' + csrf_token_name + '=' + csrf_hash;
        $.ajax({
            url: BASE_URL + 'home_ajax/save_subscriber/',
            type: 'POST',
            data: form_data,
            dataType: 'json',
            success: function (response) {
