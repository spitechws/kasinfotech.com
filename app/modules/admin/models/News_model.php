...
class News_model extends MY_Model {
    ...
    function add() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $attachment = '';
            $row = get_row('news', array("news_id" => $editId));
            if (isset($_FILES['attachment']['name']) && $_FILES['attachment']['name'] != "") {
                $attachment = upload_media('attachment', 'pdf', 'news');
                if (isset($attachment['error'])) {
                    ...
                    if (!empty($row) && trim($row->attachment) != "") {
                        remove_media($row->attachment, 'news');
                    }
                }
            } else {
            ...
            if ($response['is_error'] == '0') {
                if ($attachment != "") {
                    ...
                    $_POST['rowId'] = $editId;
                    $lastId = parent::save($this->tbl_name, $aInput, 'news_id');
                    $response['msg'] = $lastId;
                } else {
                    $response['is_error'] = 1;
                    $response['msg'] = 'News File is required';
                }
            }
        } else {
            $response['is_error'] = 1;
    ...
    function get_list($aWhere = array()) {
        ...
        $this->db->order_by("t1.news_id", 'desc');
